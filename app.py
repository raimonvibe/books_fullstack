...
class BookOperationError(Exception):
    """Custom exception for book operations"""
    pass

class BookDatabaseException(Exception):
    """Custom exception class for Book Database application"""
    pass
...
class User(UserMixin, db.Model):
    ...
    favorite_books = db.relationship('Book', secondary='favorites', back_populates='favorited_by')
...
class Book(db.Model):
    ...
    favorited_by = db.relationship('User', secondary='favorites', back_populates='favorite_books')
...
def api_books():
    try:
        if request.method == 'POST':
            ...
            db.session.commit()
            app.logger.info(f"Book added successfully: {new_book.id}")
...
def book_details(id):
    ...
    return render_template('book_details.html', book=book, is_favorite=is_favorite)
...
@app.route('/add_edit_book/<int:book_id>', methods=['GET', 'POST'])
@login_required
def add_edit_book(book_id=None):
    ...
    if request.method == 'POST':
        try:
            ...
            flash(str(e), 'error')
            return render_template('add_edit_book.html', book=book), 400
            ...
            db.session.rollback()
            app.logger.error(f"Unexpected error {'updating' if book else 'adding'} book: {str(e)}")
...
@app.route('/book/<int:book_id>/reviews', methods=['POST'])
@login_required
...
@app.route('/api/user/<int:user_id>/wishlist/<int:book_id>', methods=['DELETE'])
@login_required
